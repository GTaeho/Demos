...
#include "sdio_sd.h"
#include "ff.h"
#include "NAND_Flash\fsmc_nand.h"
#include "NAND_Flash\nand_if.h"
#include <stdio.h>
#include <string.h>
...
char path[512]="0:";
uint8_t textFileBuffer[3000] = "HELLO? CHEN";   
...
int main(void)
{
    ...
    //rdBuff[511] = 0xaa; 
    //SSTF016B_Erase(0,0);
    //SSTF016B_RD(0, rdBuff, 512);
    //FSMC_NAND_WriteSmallPage (rdBuff, WriteReadAddr, 1);
    //FSMC_NAND_ReadSmallPage (rdBuff, WriteReadAddr, 1);

	//nand_readsects(0,rdBuff,1);
	//nand_phy_readsect(0,rdBuff);
	//nand_phy_readpage(0,rdBuff);
	//NAND_Read(130*2048, (int32_t *)rdBuff,2048);
    //NAND_Fatreadsects(0,rdBuff,1);
	...
	res = f_open( &fsrc , "0:/Demo1.TXT" , FA_CREATE_NEW | FA_WRITE |FA_READ);
    ...
    { 
	  ...
	  //     textFileBuffer[index] = 'A';
	  //}
      res = f_write(&fsrc, textFileBuffer, sizeof(textFileBuffer), &br);     
	  //res = f_lseek (&fsrc, 2 );	
      f_sync (&fsrc);
 	  //res = f_read (&fsrc, buf_read, 10, &bytesWritten);
...
int SD_TotalSize(void)
{
    ...
    if ( res==FR_OK ) 
    {
      ...
      //     "%d MB available.\r\n",
      //     ( (fs->n_fatent - 2) * fs->csize ) / 512 /2048 , (fre_clust * fs->csize) / 512/2048 );
