...
/* Private variables ---------------------------------------------------------*/
uint16_t LUT[4096]; //Look Up Table Buffer
...
uint16_t NAND_Write(uint32_t Memory_Offset, uint32_t *Writebuff, uint16_t Transfer_Length)
{
  ...
  {/* Idle state */
    ...
    { /* USED BLOCK */
	  ...
	  //printf("\r\n NAND_GetFreeBlock : Zone(%d) : Block(%d) : Page(%d)\r\n",fAddress.Zone,fAddress.Block,fAddress.Page);
    ...
    {/* UNUSED BLOCK */
	  ...
	  //printf("\r\n Write : Zone(%d) : Block(%d) : Page(%d)\r\n",wAddress.Zone,wAddress.Block,wAddress.Page);
...
uint16_t NAND_Read(uint32_t Memory_Offset, uint32_t *Readbuff, uint16_t Transfer_Length)
{
  ...
  else
  {
    ...
    //printf("\r\n Read Address : Zone(%d) : Block(%d) : Page(%d)\r\n",phAddress.Zone,phAddress.Block,phAddress.Page);
...
static uint16_t NAND_BuildLUT (uint8_t ZoneNbr)
{
  ...
  *****************************************************************************/
  ...
  for (pCurrentBlock = 0 ; pCurrentBlock < MAX_PHY_BLOCKS_PER_ZONE ; pCurrentBlock++ )
  {
    ...
    //printf("%04x ",LUT[pCurrentBlock]);
    //if((pCurrentBlock%12)==0) printf("\r\n");
